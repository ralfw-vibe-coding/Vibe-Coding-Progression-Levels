...
function inTempVerzeichnis(test: (pfad: string) => void) {
  const verzeichnis = Deno.makeTempDirSync();
  try {
    test(`${verzeichnis}/depot-events.json`);
  } finally {
    Deno.removeSync(verzeichnis, { recursive: true });
  }
}

function dateiTest(name: string, fn: (pfad: string) => void) {
  Deno.test({ name, ignore: UEBERSPRINGEN, fn: () => inTempVerzeichnis(fn) });
}

dateiTest("append schreibt sofort — ohne dass jemand das Speichern anstoßen müsste", (pfad) => {
...
dateiTest("ein neuer Store auf derselben Datei setzt den Bestand fort (simulierter Neustart)", (pfad) => {
...
dateiTest("nach einem Neustart knüpft die seq-Vergabe nahtlos an", (pfad) => {
  const ersterStore = createDateiEventStore(pfad);
  ersterStore.append("kauf", { wertpapierId: "A" });
...
dateiTest("seq und timestamp überstehen den Neustart unverändert", (pfad) => {
  const ersterStore = createDateiEventStore(pfad);
  const original = ersterStore.append("kauf", { wertpapierId: "A" });

  const zweiterStore = createDateiEventStore(pfad);
  const wieder = zweiterStore.query()[0];
...
dateiTest("eine beschädigte Datei wird gemeldet, statt still als leer zu gelten", (pfad) => {
...
Deno.test({
  ...
  fn() {
    ...
    try {
      ...
      store.append("kauf", { wertpapierId: "A" });
      if (store.query().length !== 1) throw new Error("der Store muss auch in einem neuen Verzeichnis schreiben können");
    } finally {
